...
function ScoreRing({label,value}:{label:string,value:number}){
  return <div className="score"><div className="scoreValue">{value}</div><div className="scoreLabel">{label}</div></div>
}
...
export default function RadarApp(){
  ...
  async function loadLive(){
    try{
      ...
      if(j?.state?.analysis){setRemote(j.state.analysis);setMode('LIVE');setTab('Radar')}
      else alert('Ainda não existe feed real conectado. O sistema continuará em DEMO/MANUAL.')
    }catch{alert('Não foi possível consultar o estado ao vivo.')}
  }

  function saveManual(next:MarketSnapshot){
    const s={...next,source:'MANUAL' as const,timestamp:new Date().toISOString()}
    setSnapshot(s);localStorage.setItem('integraradar-manual',JSON.stringify(s));setMode('MANUAL');setTab('Radar')
  }
  ...
  return <main className="shell">
    <header className="topbar">
      ...
      <button className="tiny" onClick={loadLive}>Atualizar</button>
    </header>

    <section className="modebar">
      <Badge tone={mode==='DEMO'?'warn':mode==='LIVE'?'good':'info'}>{mode}</Badge>
      <span>{snapshot.asset}</span><span>•</span><span>{new Date(mode==='LIVE'&&remote?remote.timestamp:snapshot.timestamp).toLocaleString('pt-BR')}</span>
    </section>
    ...
    {tab==='Radar'&&<>
      ...
      <section className="disclaimer">O IntegraRadar é um sistema de apoio à decisão e gestão de risco. Não envia ordens e não garante resultado.</section>
    </>}

    {tab==='Entrada'&&<ManualForm initial={snapshot} onSave={saveManual} onDemo={()=>{setSnapshot({...demoSnapshot,timestamp:new Date().toISOString()});setMode('DEMO');setTab('Radar')}}/>}
    {tab==='Sinais'&&<Signals/>}
    {tab==='Diário'&&<Journal/>}
...
function Integrations(){return <section className="panel"><h2>Integrações</h2>{[
  ['Toro Trader Mobile','Execução manual','REAL'],['Supabase','Banco e sinais','PENDENTE'],['Vercel','Hospedagem do app','PENDENTE'],['GitHub','Código-fonte','CONECTADO'],['TradingView / feed B3','Feed futuro','NÃO CONECTADO'],['Massive','Contexto internacional','CONECTADO'],['Hugging Face','Pesquisa/modelos futuros','DISPONÍVEL']
].map(([a,b,c])=><div className="integration" key={a}><div><b>{a}</b><span>{b}</span></div><Badge tone={c==='REAL'||c==='CONECTADO'?'good':c==='PENDENTE'?'warn':'neutral'}>{c}</Badge></div>)}<div className="card missing"><b>Regra de integridade</b><p>Nunca converter dado atrasado, DEMO ou ausente em “tempo real”. A origem é sempre exibida.</p></div></section>}
